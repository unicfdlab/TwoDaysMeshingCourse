...
/* number of possible colors in texture mode */
#define     TEX_PIXELS  512

/* gamma lightens or darkens an image (Hcpy) */
#define     GAMMA       1.9     
...
/* amplification factor for the automatic merging of edge-points if a cad-file is read */
#define     GTOL_EDGES  2.e1


/* defaults for calcLineDiv() */
/* defines the minimum angle between sectors of a line in calcLineDiv() */
#define GTOL_COS_A 0.998      

/* defines the default maximum distance between nodes during automatic line division calculation */
/* GTOL_NODE_DIST * gtol = target distance between nodes (not exactly achieved) */
/* A line running from one end to the other end of the model would have a division of  GTOL*GTOL_NODE_DIST */ 
#define GTOL_NODE_DIST 1.e3

/* defines the default minimum element length/maximum element length */
#define     ELEM_LENGTH_RATIO 0.1

/* default line division for meshing */
#define     DEF_LINE_DIV 4      
...
/* default length of one period of animation */
...
/* default location of the help files, please insert the actual one */
  ...
  #else
    #define     HELPFILE        {"/yaprod/cae/application/CalculiX/doc/cgx_2.6/cgx.html",\
                             "/yaprod/cae/application/CalculiX/doc/ccx_2.6/ccx.html"}
  #endif
#else
...
#define     MIN_ANGLE_TRI3  0.999   /* badelems.c: S*S, alfa=acos(sqrt(MIN_ANGLE_TRI3)) */
...
#define     FLPB          "-FLPB"   /* stores bodies which will be inverse oriented */ 
#define     NJBY          "-NJBY"   /*  */ 
#define     COPY          "-COPY"   /*  */
#define     MESH          "-MESH"   /*  */
...
/* writefbd.c */
int writefbd(char *setname, Summen *anz, SumGeo *anzGeo, Points *pnt, Lines *line, Lcmb *lcmb, Gsur *surf, Gbod *body, Nurbs *nurs, Sets *set, Shapes *shape );
...
void bak_delPnt( int anz, int *number );
...
void sendPressure( char *setname, char *format, Summen *anz, Nodes *node, Elements *e_enqire, char *value, char *val1, char *val2 );
...
void trimNurbs( Nurbs *nurbs, int nr, int patch, double);
