...
void sendPressure( char *setname, char *format, Summen *anz, Nodes *node, Elements *e_enqire, char *val1, char *val2, char *val3 )
{
  int   setNr;
  int  i,j;
  char prognam[MAX_LINE_LENGTH];
  ...
  if(compare(val1, "ds", 2)==2)  sprintf(prognam,"%s_%s%s.dlo",setname,val1,val2);
  else sprintf(prognam,"%s.dlo",setname);
  printf (" write file: %s\n", prognam );
      ...
      if(e_enqire[face[i].elem_nr].type>6)  // shell
      {
        ...
        else if(e_enqire[face[i].elem_nr].attr>3)  // plain strain,stress or axissym
        {
          ...
          if((face[i].nr==1)&&(orif==1)) fprintf( handle, "%d, PP, %lf\n", face[i].elem_nr, pressure[j]);
                  ...
                  */
           fprintf( handle, "%d, P%d, %lf\n", face[i].elem_nr, face[i].nr-1, pressure[j]);
        }
        else
        {
          if(face[i].nr==1) fprintf( handle, "%d, P, %lf\n", face[i].elem_nr, pressure[j]);
...
void sendSPC( char *setname, char *format, Summen *anz, Nodes *node, Elements *e_enqire, char *value, char *val1, char *val2, char *val3 )
{
  ...
  sprintf(prognam,"%s_%s%s%s%s.bou",setname,value,val1,val2,val3);
  printf (" write file: %s\n", prognam );
  ...
  if (compare( format, "abq", 3)== 3)
  {
    ...
    else
    {
     ...
     for (i=0; i<length; i++)
     {
      ...
      if ((value[i]!='t')&&((dofi<0)||(dofi>6))) { errMsg ("ERROR dof:%s not known\n", dofa); return; }
      else 
      {
        /* check if a constant value or a dataset has to be used */
        if(compare(val1, "ds", 2)==2)
	{
          ...
          {
            fprintf(handle, "%d, %1c, , %e\n",set[setNr].node[j], value[i],  lcase[lc].dat[entity][set[setNr].node[j]] );
          }
	}
        else
	{
          for (j=0; j<set[setNr].anz_n; j++ )
          {
            if( value[i]=='t') fprintf(handle, "%d, %d, , %s\n",set[setNr].node[j], DOFT, val1  );
            else fprintf(handle, "%d, %1c, , %s\n",set[setNr].node[j], value[i], val1  );
          }
	}
      }
     }
...
void sendCflux( char *setname, char *format, Summen *anz, Nodes *node, Elements *e_enqire, char *value, char *val2 )
{
  ...
  if (compare( format, "abq", 3)== 3)
  {
    ...
    for (i=0; i<length; i++)
    {
      if (value[i]!='t') errMsg ("ERROR dof:%d not known\n", dofi);
      else 
      {
        for (j=0; j<set[setNr].anz_n; j++ )
        {
