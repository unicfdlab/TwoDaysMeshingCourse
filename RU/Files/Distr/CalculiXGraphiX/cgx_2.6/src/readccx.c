...
int getNodes(char *rec_str)
{
  ...
  do{
    ...
    /* check if that node is already existend */
    /*
    nodex[n].nx=x;
    nodex[n].ny=y;
    nodex[n].nz=z;
...
int getCload(char *rec_str)
{
  ...
  if(!step_flag) { printf(" ERROR: load was defined outside a *STEP\n\n"); exit(0); }
...
int readccx(char *datin, Summen *apre, Sets **sptr, Nodes **nptr, Elements **eptr, Datasets **lptr )
{
  ...
  do
  {
    ...
    /* get the heading */
    else if (compare(dat[0], "*HEADING",7)==7) { length=abqrecord(rec_str); strcpy(command,"TEXT"); commandoInterpreter( command, rec_str, 0, 0, 0, 0, 0 ); goto checknextstr; }
  ...
  */
  ...
  for (i=0; i<anzx->l; i++) { lcasex[i].fileptr=0; lcasex[i].loaded=1; }
    ...
    for(j=0; j<lcasex[l].ncomps; j++)
    {
      ...
      if(lcasex[l].iexist[j] <0)
      {
        ...
        for (i=0; i<anzx->n; i++)
        {
          ...
          {
            for(j=0; j<nr; j++) lcasex[l].dat[j][nodex[i].nr]/= lcasex[l].dat[nr][nodex[i].nr];
