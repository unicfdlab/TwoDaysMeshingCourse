...
/* todo:
The shape sets used in transformations do not include their initial coorinate axes. This might be necessary to change in future. 
...
void make_manifoldSet(Step_Object *step_object, int indx)
{
  ...
  for (i=0; i< nf; i++)
  {
    ...
    if(strstr(step_object[face].class_name, "FACE") !=NULL)
    {
      ...
      /* get the bounds */
      nb=get_param_in_string(step_object[face].param[1], &bounds, &pos);
